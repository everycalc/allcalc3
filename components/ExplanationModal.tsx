...
const ExplanationModal: React.FC<ExplanationModalProps> = ({ isOpen, onClose, calculatorName, inputs, result }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
      ...
      />
      <div
        className="modal-content relative w-full max-w-2xl rounded-xl shadow-2xl flex flex-col"
      ...
      >
        <header className="flex justify-between items-center p-4 border-b border-outline-variant">
          <h2 id="explanation-modal-title" className="text-xl font-bold text-on-surface">Calculation Explained</h2>
          <button onClick={onClose} className="p-2 rounded-full hover:bg-surface-container-high text-on-surface-variant" aria-label="Close explanation">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
          </button>
        </header>
        <main className="p-6 max-h-[60vh] overflow-y-auto">
            {isLoading && (
                <div className="flex items-center justify-center space-x-2 text-on-surface">
            ...
            {!isLoading && !error && explanation && (
                <div className="prose" dangerouslySetInnerHTML={{ __html: explanation }} />
            )}
        </main>
      </div>
    </div>
