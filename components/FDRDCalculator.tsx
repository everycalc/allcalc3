        ...
        <div className="space-y-6">
            ...
            {isExplainModalOpen && result && (
                <ExplanationModal
                    ...
                    calculatorName={`${depositType} Calculator`}
                    inputs={{ depositType, investment, rate, tenure, compounding }}
                    result={result}
                />
            )}
             <div className="flex justify-center bg-theme-primary p-1 rounded-lg">
                <button onClick={() => handleTypeChange('FD')} className={`w-1/2 py-2 rounded-md transition ${depositType === 'FD' ? 'bg-primary text-on-primary' : 'hover:bg-theme-tertiary'}`}>Fixed Deposit</button>
                <button onClick={() => handleTypeChange('RD')} className={`w-1/2 py-2 rounded-md transition ${depositType === 'RD' ? 'bg-primary text-on-primary' : 'hover:bg-theme-tertiary'}`}>Recurring Deposit</button>
            </div>
            <div className="space-y-4">
                <div>
                     <div className="flex items-center space-x-2 mb-1">
                        ...
                        <InfoTooltip text={depositType === 'FD' ? "A one-time lump sum investment." : "A fixed amount invested each month."} />
                    </div>
                    <input type="number" id="investment" value={investment} onChange={e => setInvestment(e.target.value)} className={inputClasses}/>
                </div>
                <div>
                     <div className="flex items-center space-x-2 mb-1">
                        <label htmlFor="rate" className="block text-sm font-medium text-theme-secondary">Annual Interest Rate (%)</label>
                        <InfoTooltip text="The rate at which your deposit earns interest per year." />
                    </div>
                ...
                <div>
                 ...
                 {depositType === 'FD' && <div>
                     <div className="flex items-center space-x-2 mb-1">
                        ...
                        <InfoTooltip text="How often the earned interest is added back to the principal amount." />
                    </div>
