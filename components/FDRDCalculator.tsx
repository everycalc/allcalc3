...
const FDRDCalculator: React.FC<FDRDCalculatorProps> = ({ initialState, isPremium }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="toggle-group flex justify-center p-1 rounded-lg">
                <button onClick={() => handleTypeChange('FD')} className={`toggle-button w-1/2 py-2 rounded-md transition ${depositType === 'FD' ? 'toggle-button-active' : ''}`}>Fixed Deposit (FD)</button>
                <button onClick={() => handleTypeChange('RD')} className={`toggle-button w-1/2 py-2 rounded-md transition ${depositType === 'RD' ? 'toggle-button-active' : ''}`}>Recurring Deposit (RD)</button>
            </div>
            <div className="space-y-4">
                <div>
                    <div className="flex items-center space-x-2 mb-1">
                        <label htmlFor="investment" className="block text-sm font-medium text-on-surface-variant">{depositType === 'FD' ? 'Total Investment' : 'Monthly Investment'} ({currencySymbol})</label>
                        <InfoTooltip text={depositType === 'FD' ? "A one-time lump sum investment." : "The amount you will invest each month."} />
                    </div>
                    <input type="number" id="investment" value={investment} onChange={e => setInvestment(e.target.value)} className="input-base w-full"/>
                </div>
                <div>
                    <div className="flex items-center space-x-2 mb-1">
                        <label htmlFor="rate" className="block text-sm font-medium text-on-surface-variant">Interest Rate (% p.a.)</label>
                ...
                <div>
                    <label htmlFor="tenure" className="block text-sm font-medium text-on-surface-variant mb-1">Tenure (Years)</label>
                    <input type="number" id="tenure" value={tenure} onChange={e => setTenure(e.target.value)} className="input-base w-full"/>
                </div>
                {depositType === 'FD' && (
                    <div>
                        <div className="flex items-center space-x-2 mb-1">
                            <label htmlFor="compounding" className="block text-sm font-medium text-on-surface-variant">Compounding Frequency</label>
                            <InfoTooltip text="How often the interest is calculated and added to the principal." />
                        </div>
                        <select id="compounding" value={compounding} onChange={e => setCompounding(e.target.value)} className="select-base w-full">
            ...
            {result && (
                <div className="result-card p-6 space-y-4 animate-fade-in">
                    ...
                    <div className="flex justify-between items-center">
                     ...
                     <div className="flex justify-between items-center">
                        <span className="text-on-surface-variant">Interest Earned:</span>
                        <span className="text-lg font-medium text-on-surface">{formatCurrency(result.interestEarned)}</span>
                    </div>
