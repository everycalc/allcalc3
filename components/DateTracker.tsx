                        ...
                        {savedDates.map(item => (
                            <li key={item.id} className="bg-surface-container p-3 rounded-lg flex justify-between items-center list-none">
                                <div>
                                    ...
                                    <p className="text-xs text-on-surface-variant">{new Date(item.date).toLocaleDateString(undefined, { year: 'numeric', month: 'long', day: 'numeric', timeZone: 'UTC' })} ({item.type})</p>
                                </div>
