...
const SavedRecipeList: React.FC<{onLoad: (recipe: Recipe) => void}> = ({ onLoad }) => {
    ...
    return (
        <div className="mt-8">
            <h3 className="text-xl font-semibold text-on-surface mb-4">Saved Recipes</h3>
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2">
                {savedRecipes.map(recipe => (
                    <button 
                        key={recipe.id}
                        onClick={() => onLoad(recipe)}
                        className="w-full text-left bg-surface-container p-3 rounded-lg hover:bg-surface-container-high transition-colors"
                    >
...
const RecipeCostCalculator: React.FC<{initialState?: Recipe, isPremium?: boolean}> = ({initialState, isPremium}) => {
        ...
        <div className="space-y-4">
            <div className="bg-surface-container p-4 rounded-lg space-y-4">
                <input 
                    ...
                    className="text-xl font-bold bg-transparent focus:outline-none w-full text-on-surface border-b-2 border-transparent focus:border-primary transition-colors" 
                ...
                />
                 ...
                 <div className="space-y-2">
                    {recipe.ingredients.map(ing => {
                       ...
                       return (
                         <details key={ing.id} open={isActive} className="bg-surface-container-low rounded-lg overflow-hidden" onToggle={(e) => { if (e.currentTarget.open) setActiveIngredientId(ing.id); }}>
                            <summary className="p-3 cursor-pointer flex justify-between items-center list-none" onClick={(e) => { e.preventDefault(); setActiveIngredientId(isActive ? null : ing.id); }}>
                                <span className="font-semibold text-on-surface truncate pr-2">{ing.name || 'New Ingredient'}</span>
                                <div className="flex items-center space-x-2 flex-shrink-0">
                                    ...
                                    <svg xmlns="http://www.w3.org/2000/svg" className={`h-5 w-5 text-on-surface-variant transition-transform ${isActive ? 'rotate-180' : ''}`} viewBox="0 0 20 20" fill="currentColor"><path fillRule="evenodd" d="M5.293 7.293a1 1 0 011.414 0L10 10.586l3.293-3.293a1 1 0 111.414 1.414l-4 4a1 1 0 01-1.414 0l-4-4a1 1 0 010-1.414z" clipRule="evenodd" /></svg>
                                </div>
                            </summary>
                            <div className="p-3 border-t border-surface-container space-y-2">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                    <div><label className="text-xs text-on-surface-variant flex items-center mb-1">Price <InfoTooltip text="Price for the whole package." /></label><input type="number" placeholder="e.g., 50" value={ing.marketPrice} onChange={e => handleIngredientChange(ing.id, 'marketPrice', e.target.value)} className={itemInputClasses} /></div>
                                    <div><label className="text-xs text-on-surface-variant flex items-center mb-1">Size <InfoTooltip text="Package size (g, kg, ml, etc)." /></label><input type="number" placeholder="e.g., 1000" value={ing.packageSize} onChange={e => handleIngredientChange(ing.id, 'packageSize', e.target.value)} className={itemInputClasses} /></div>
                                    <div><label className="text-xs text-on-surface-variant flex items-center mb-1">Used <InfoTooltip text="Amount used in recipe." /></label><input type="number" placeholder="e.g., 500" value={ing.usedAmount} onChange={e => handleIngredientChange(ing.id, 'usedAmount', e.target.value)} className={itemInputClasses} /></div>
                                </div>
                            </div>
                         </details>
                       )
                ...
                <button onClick={handleAddIngredient} className="w-full border-2 border-dashed border-outline-variant text-on-surface-variant font-semibold py-2 px-4 rounded-md hover:bg-surface-container-high hover:text-on-surface transition-colors text-sm">+ Add Ingredient</button>

                <div className="flex justify-between items-center border-t border-outline-variant pt-3 mt-3">
                    <div className="text-right flex-grow">
                        <h4 className="text-sm font-semibold text-on-surface-variant">Total Recipe Cost</h4>
            ...
            <div className="flex justify-between items-center pt-2">
                <button onClick={handleSaveToHistory} className="btn-primary font-bold py-2 px-4 rounded-md shadow-lg">Save Recipe</button>
