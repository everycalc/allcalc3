...
const DiscountCalculator: React.FC<DiscountCalculatorProps> = ({ initialState, isPremium }) => {
            ...
            <div className="space-y-4">
                <div>
                    <div className="flex items-center space-x-2 mb-1">
                        <label htmlFor="originalPrice" className="block text-sm font-medium text-on-surface-variant">Original Price ({currencySymbol})</label>
                        <InfoTooltip text="The price of the item before any discount." />
                    </div>
                    <input type="number" id="originalPrice" value={originalPrice} onChange={(e) => setOriginalPrice(e.target.value)} className="input-base w-full"/>
                </div>
                <div>
                    <div className="flex items-center space-x-2 mb-1">
                        <label htmlFor="discount" className="block text-sm font-medium text-on-surface-variant">Discount (%)</label>
                        <InfoTooltip text="The percentage discount to be applied." />
                    </div>
                    <input type="number" id="discount" value={discount} onChange={(e) => setDiscount(e.target.value)} className="input-base w-full"/>
                </div>
            </div>
            ...
            {result && (
                <div className="result-card p-6 space-y-4 animate-fade-in">
                    ...
                    <div className="flex justify-between items-center">
                        <span className="text-on-surface-variant">You Save:</span>
                    ...
                    <div className="flex justify-between items-center border-t border-outline-variant pt-4 mt-4">
                        <span className="text-on-surface-variant">Final Price:</span>
