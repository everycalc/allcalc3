...
interface CalculatorCarouselProps {
  ...
  pinTargetName?: string;
}

const CalculatorCarousel: React.FC<CalculatorCarouselProps> = ({ items, onSelectCalculator, pinTargetName }) => {
        ...
        {items.map(item => (
          <div key={item.name} className="flex-shrink-0 w-24" style={{ scrollSnapAlign: 'start' }}>
            <CalculatorCard
              ...
              pinId={item.name === pinTargetName ? 'onboarding-pin-target' : undefined}
            />
          </div>
        ))}
      </div>
