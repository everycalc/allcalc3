...
const ThemeModal: React.FC<ThemeModalProps> = ({ isOpen, onClose, onOpenLayoutModal }) => {
  const [isThemeGuideOpen, setIsThemeGuideOpen] = useState(false);

  useEffect(() => {
    if (isOpen) {
      document.body.style.overflow = 'hidden';
      const hasSeenThemeGuide = sessionStorage.getItem('hasSeenThemeGuide');
      if (!hasSeenThemeGuide) {
        setIsThemeGuideOpen(true);
      }
    } else {
  ...
  const handleCloseGuide = () => {
    setIsThemeGuideOpen(false);
    sessionStorage.setItem('hasSeenThemeGuide', 'true');
  };

  return (
    <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-fade-in">
      <ThemeGuide isOpen={isThemeGuideOpen} onClose={handleCloseGuide} />
      <div 
