    ...
    return (
        <div className="flex flex-col min-h-screen">
            ...
            <main className="flex-grow p-4 max-w-2xl mx-auto w-full">
                <AdsensePlaceholder />
                <div className="bg-surface-container p-6 rounded-lg text-center mt-6">
                    <h2 className="text-2xl font-bold text-primary">Come back every day!</h2>
                    <p className="text-on-surface-variant mt-2">Maintain your streak to earn bigger Calculation Fuel rewards. Your current streak is:</p>
                    <p className="text-6xl font-extrabold text-on-surface my-4">{streak} {streak === 1 ? 'Day' : 'Days'}</p>
                    ...
                    <div className="grid grid-cols-4 sm:grid-cols-7 gap-2 my-6">
                        {REWARDS.map((reward, index) => {
                            ...
                            return (
                                <div key={dayNumber} className={`p-3 rounded-lg border-2 ${isCurrent ? 'border-primary animate-pulse' : isClaimed ? 'border-green-500 bg-green-500/10' : 'border-outline-variant'}`}>
                                    <p className="text-xs font-bold text-on-surface-variant">Day {dayNumber}</p>
                    ...
                    <button 
                        ...
                        className="btn-primary w-full font-bold py-3 px-4 rounded-lg shadow-lg disabled:opacity-50 disabled:cursor-not-allowed"
                    >
