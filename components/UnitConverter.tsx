  ...
  const handleConversion = () => {
    ...
    if (fuel >= fuelCost) {
        ...
        if (shouldShowAd(isPremium)) {
            setPendingCalculation(() => performConversion);
            setShowAd(true);
        } else {
            performConversion();
        }
  ...
  return (
    <div className="space-y-6">
      ...
      <div>
        ...
        <select id="category" value={category} onChange={e => setCategory(e.target.value as Category)} className="select-base w-full">
      ...
      <div className="flex items-center space-x-4">
        <div className="w-full">
          <label htmlFor="fromUnit" className="block text-sm font-medium text-on-surface-variant mb-1">From</label>
          <select id="fromUnit" value={fromUnit} onChange={e => setFromUnit(e.target.value)} className="select-base w-full">
            {units.map(unit => <option key={unit}>{unit}</option>)}
          </select>
          <input type="number" value={inputValue} onChange={e => setInputValue(e.target.value)} className="input-base w-full mt-2" />
        </div>

        <button onClick={handleSwap} className="btn-secondary mt-8 p-3 rounded-full transition">
        ...
        <div className="w-full">
          <label htmlFor="toUnit" className="block text-sm font-medium text-on-surface-variant mb-1">To</label>
          <select id="toUnit" value={toUnit} onChange={e => setToUnit(e.target.value)} className="select-base w-full">
            {units.map(unit => <option key={unit}>{unit}</option>)}
          </select>
          <input type="text" value={outputValue} readOnly className="input-base w-full mt-2 cursor-not-allowed" placeholder="Result" />
        </div>
      </div>
      <div className="text-center">
        <button onClick={handleConversion} className="btn-primary font-bold py-3 px-8 rounded-full hover:shadow-lg transition-shadow duration-200 shadow-md">
            Convert
        </button>
      </div>
